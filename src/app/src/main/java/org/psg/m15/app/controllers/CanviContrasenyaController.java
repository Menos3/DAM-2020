...
import org.psg.m15.app.classes.*;
...
public class CanviContrasenyaController extends DefaultDBController implements Initializable {

    public static final int MAX_CARACTERS = 6;
...
//        pass=eh.AESEncrypt(tfContrasenyaActual.getText());
    ...
    private boolean passwordAccepted() throws IllegalBlockSizeException, InvalidKeyException, BadPaddingException, NoSuchAlgorithmException, NoSuchPaddingException {
        ...
        String oldPassEnc = eh.AESEncrypt(pfContrasenyaActual.getText());
    ...
    private boolean testNewPassword() {
        ...
        if (pfNovaContrasenya.getText().isEmpty()) {
            ...
            Utilitats.AlertaGeneralWarning(Constants.TITOL_CANVI_CONTRASENYA, Constants.CAPCALERA_ALERTA_CAMPS_BUITS, Constants.MISSATGE_ALERTA_CAMPS_BUITS);
            result = false;
        } else if (pfContrasenyaActual.getLength() <= MAX_CARACTERS || pfNovaContrasenya.getLength() <= MAX_CARACTERS || pfConfirmaContrasenya.getLength() <= MAX_CARACTERS) {
            Utilitats.AlertaGeneralWarning(Constants.TITOL_CANVI_CONTRASENYA, Constants.CAPCALERA_ALERTA_LONGITUD, "La longitud minima ha de ser de" + MAX_CARACTERS);
            result = false;
    ...
    private void actualitzaBD(String contrasenya) {
        ...
        try {
            ...
            if (!usuari.getPasswd().equals(contrasenya)) {
                pass = eh.AESEncrypt(contrasenya);
                u.setPasswd(pass);
                helper.update(u);
