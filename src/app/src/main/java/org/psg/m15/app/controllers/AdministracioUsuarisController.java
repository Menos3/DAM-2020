...
import org.psg.m15.app.classes.GMailHelper;
...
public class AdministracioUsuarisController extends DefaultDBController implements Initializable {
    ...
    private Modes mode = Modes.CERCA;
            ...
            row.setOnMouseClicked(event -> {
                if (event.getClickCount() == Constants.DOUBLE_CLICK && (!row.isEmpty())) {
                    ...
                    if (!u.isAdmin()) {
                        ...
                        if (u.isBloquejat()) {
                            GMailHelper.send(u.getEmail(), Constants.ASSUMPTE_CORREU_BLOQUEIG, Constants.COS_CORREU_BLOQUEIG_ADMIN);
                        }
    ...
    private void configurarColumnes() {
        ...
        tcBloquejat.setCellValueFactory(dadesCella -> {
            Usuari valorCella = dadesCella.getValue();
            BooleanProperty property = new SimpleBooleanProperty();
            property.setValue(valorCella.isBloquejat());
            return property;
        });
    }
    ...
    private void accionsPerModificar() {
        ...
        try {
            ...
            Logger.getLogger(Main.class.getName()).log(Level.SEVERE, "Error al iniciar", ie);
        }
    }
    ...
    public void btEliminarUsuariOnAction(ActionEvent event) {
        ...
        alert.setHeaderText(Constants.CONFIRMAR_ACCIO);
        alert.setContentText(Constants.SEGUR_QUE_VOLS_ELIMINAR + "\n\n " +
